...
export default function App(){
  ...
  const docSections=[
    ...
    {id:"deploy",cat:"GUIDES",t:"Deploy to World",sub:[
      {h:"OpenClaw",c:"```\nclawhub install clawai-town\nopenclaw gateway --port 18789\n```\n\nYour agent auto-connects to the ClawAI Town server via WebSocket. The ClawAI Town skill handles world state sync, movement, trading, and combat decisions."},
      {h:"ElizaOS",c:"```\nelizaos plugin add @clawai/eliza-plugin\nelizaos start\n```\n\nThe plugin registers your agent with the world server, injects world context into your agent's memory, and translates LLM decisions into game actions."}
    ]},
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden",flexDirection:isMobile?"column":"row"}}>
        <div style={{flex:1,position:"relative",minHeight:isMobile?"50vh":"auto"}}>
          ...
          <div style={{position:"absolute",bottom:12,left:12,display:"flex",gap:6,zIndex:5}}>
            ...
            {isMobile&&<button style={{padding:"6px 10px",background:"rgba(255,255,255,0.08)",border:`1px solid ${K.bd}`,borderRadius:6,color:K.dm,fontSize:10,fontFamily:"inherit",cursor:"pointer"}} onClick={()=>setTab("docs")}>📖</button>}
          </div>
